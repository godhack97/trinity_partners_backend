...
export class UserActionsController {
  ...
  async getPaged(
    ...
    if (action) {
      return this.userActionsService.findPagedByAction(action, Number(skip), Number(take));
    }

    const { logs, total } = await this.userActionsService.findPaged(Number(skip), Number(take));
    return { total, logs };
  }
}
