...
export class AdminConfiguratorComponentService {
  constructor(private readonly cnfComponentRepository: CnfComponentRepository) {}
  ...
  async updateComponent(id: string, data: Partial<CreateConfigurationComponentRequestDto>) {
    const component = await this.cnfComponentRepository.findOneByOrFail({id})
    component.update(data)
    return this.createOrUpdate(component, data)
  }
  async createOrUpdate(component: CnfComponentEntity, data: Partial<CreateConfigurationComponentRequestDto>) {
    if(data.slots?.length > 0) {
      component.slots = data.slots.map((item: Partial<CnfComponentSlotEntity>) => {
