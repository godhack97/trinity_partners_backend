...
export class AdminConfiguratorServerGenerationService {
  ...
  async addServerGeneration({ name }: AddServerGenerationRequestDto) {
    ...
    if (serverGeneration) {
      throw new HttpException(
        `Поколение сервера ${name} уже существует`,
  ...
  async updateServerGeneration(
    id: string,
    { name }: AddServerGenerationRequestDto,
  ) {
    const serverGeneration = await this.cnfServerGenerationRepository.findOneBy(
      { id },
    );
    ...
    if (!serverGeneration) {
      throw new HttpException(
        `Поколение сервера ${id} не найдено`,
        HttpStatus.NOT_FOUND,
      );
    }

    const serverGenerationName =
      await this.cnfServerGenerationRepository.findOneBy({ name });
    ...
    const updatedServerGeneration =
      await this.cnfServerGenerationRepository.update(id, {
        name,
      });

    if (updatedServerGeneration.affected === 0) {
      throw new HttpException(
        "Не удалось обновить поколение сервера",
  ...
  async removeServerGeneration(id: string) {
    const removedServerGeneration =
      await this.cnfServerGenerationRepository.delete(id);

    if (removedServerGeneration.affected === 0) {
      throw new HttpException(
        "Не удалось удалить поколение сервера",
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
