...
export class AdminConfiguratorMultislotService {
  ...
  async createMultislot(data: CreateMultislotRequestDto) {
    ...
    await this.cnfMultislotSlotRepository.save(
      data.slotIds.map((el) => ({
        id: createUUID(),
    ...
    return await this.multislotBuilder()
      .where("m.id = :id", { id: multislot.id })
      .getOne();
  }
  ...
  async updateMultislot(id: string, data: UpdateMultislotRequestDto) {
    ...
    await this.cnfMultislotSlotRepository.delete({
      multislot_id: existsMultislot.id,
    });
