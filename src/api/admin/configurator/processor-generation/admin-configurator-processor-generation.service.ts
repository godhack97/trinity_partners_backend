...
export class AdminConfiguratorProcessorGenerationService {
  ...
  async addProcessorGeneration({ name }: AddProcessorGenerationRequestDto) {
    const processorGeneration =
      await this.cnfProcessorGenerationRepository.findOneBy({ name });

    if (processorGeneration) {
      throw new HttpException(
        `Поколение процессора ${name} уже существует`,
  ...
  async updateProcessorGeneration(
    id: string,
    { name }: AddProcessorGenerationRequestDto,
  ) {
    const processorGeneration =
      await this.cnfProcessorGenerationRepository.findOneBy({ id });

    if (!processorGeneration) {
      throw new HttpException(
        `Поколение процессора ${id} не найдено`,
        HttpStatus.NOT_FOUND,
      );
    }

    const processorGenerationName =
      await this.cnfProcessorGenerationRepository.findOneBy({ name });
    ...
    const updatedProcessorGeneration =
      await this.cnfProcessorGenerationRepository.update(id, {
        name,
      });

    if (updatedProcessorGeneration.affected === 0) {
      throw new HttpException(
        "Не удалось обновить поколение процессора",
  ...
  async removeProcessorGeneration(id: string) {
    const removedProcessorGeneration =
      await this.cnfProcessorGenerationRepository.delete(id);

    if (removedProcessorGeneration.affected === 0) {
      throw new HttpException(
        "Не удалось удалить поколение процессора",
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
